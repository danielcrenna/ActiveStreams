...
namespace ActiveStreams.Memory
{
	public class FileMemoryProvider<T> : IExternalMemoryProvider<T>
	{
		...
		{
			return File.OpenWrite(GetFilePathForSegment(label, index));
		}
		...
		public IEnumerable<T> Read(string label, int index, IComparer<T> sort, ISafeLogger logger = null,
			IMetricsHost metrics = null, CancellationToken cancellationToken = default)
		{
			return Read(File.OpenRead(GetFilePathForSegment(label, index)), sort, logger, metrics, cancellationToken);
		}
