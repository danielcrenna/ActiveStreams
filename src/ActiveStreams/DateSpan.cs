...
namespace ActiveStreams
{
	...
	public struct DateSpan
	{
		...
		private void CalculateSeconds(DateTime start, DateTime end)
		{
			...
			if (end.Second < start.Second) Seconds = 60 - start.Second + end.Second;
		}
		...
		private void CalculateMinutes(DateTime start, DateTime end)
		{
			...
			if (end.Minute < start.Minute) Minutes = 60 - start.Minute + end.Minute;

			if (Minutes <= 0) return;

			if (end.Second < start.Second) Minutes--;
		}
		...
		private void CalculateHours(DateTime start, DateTime end)
		{
			...
			if (end.Hour < start.Hour) Hours = 24 - start.Hour + end.Hour;
		...
		private void CalculateDays(DateTime start, DateTime end, bool excludeEndDate = false)
		{
			...
			if (end.Day < start.Day) Days = DateTime.DaysInMonth(start.Year, start.Month) - start.Day + end.Day;
			...
			else if (end.Hour == start.Hour)
			{
				if (end.Minute >= start.Minute)
				{
					if (end.Minute == start.Minute && end.Second < start.Second) Days--;
				}
				else
					Days--;
			}
		...
		private void CalculateMonths(DateTime start, DateTime end)
		{
			...
			if (end.Month < start.Month || end.Month <= start.Month && Years > 1) Months = 12 - start.Month + end.Month;
			...
			else if (end.Day == start.Day)
			{
				...
				else if (end.Hour == start.Hour)
				{
					if (end.Minute >= start.Minute)
					{
						if (end.Minute != start.Minute || end.Second >= start.Second) return;
			...
			else if (end.Month == start.Month)
			{
				if (end.Day >= start.Day)
				{
					...
					else if (end.Hour == start.Hour)
					{
						if (end.Minute >= start.Minute)
						{
							if (end.Minute != start.Minute) return;

							if (end.Second >= start.Second) return;
