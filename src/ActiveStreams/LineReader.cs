...
using Metrics;
using TypeKitchen;
using Constants = ActiveStreams.Internal.Constants;
...
namespace ActiveStreams
{
	public static class LineReader
	{
		...
		public static long ReadLines(Stream stream, Encoding encoding, NewLine onNewLine, ISafeLogger logger = null,
			IMetricsHost metrics = null, CancellationToken cancellationToken = default)
		{
			return ReadOrCountLines(stream, encoding, Constants.Buffer, onNewLine, logger, metrics, cancellationToken);
		}
		...
		private static readonly ConcurrentDictionary<int, byte[]> PendingStreams =
		...
		private static int ReadAhead(Stream stream, byte[] workingBytes, int minimumSize, int save, ref int from,
			ref int to, ref bool endOfStream, CancellationToken cancellationToken)
		{
