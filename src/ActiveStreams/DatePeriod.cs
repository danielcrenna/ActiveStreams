...
namespace ActiveStreams
{
	...
	public struct DatePeriod
	{
			...
			bool skipWeekends = true)
		{
			if (skipWeekends)
			{
				if (occurrence.DayOfWeek == DayOfWeek.Saturday) occurrence = calendar.AddDays(occurrence, 2);
				if (occurrence.DayOfWeek == DayOfWeek.Sunday) occurrence = calendar.AddDays(occurrence, 1);
			}
